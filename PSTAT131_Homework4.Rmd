...
```{r}
log_fit <- fit_resamples(log_workflow, titanic_folds)
```

```{r}
lda_fit <- fit_resamples(lda_workflow, titanic_folds)
```

```{r}
qda_fit <- fit_resamples(qda_workflow, titanic_folds)
```
...
Choose LDA; slightly lower mean accuracy (by 0.01), but lower standard error by 0.04.
